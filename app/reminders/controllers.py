...
reminders = api.namespace('api/v1.0/reminders',
                          description='CRUD operation for syc reminders')
...
class Requests_reminderById(Resource):
    ...
    def put(self, reminder_id):
        ...
        reminder.text = request.get_json()['text'] if request.get_json()[
            'text'] else reminder.text
        db.session.commit()
        return jsonify(reminder_schema.dump(reminder))
...
class General_reminder_requests(Resource):
    ...
    def get(self):
        ...
        if request.args.get('reminder_id'):
            ...
            pages = reminders_count // app.config['PER_PAGE'] + \
                (reminders_count % app.config['PER_PAGE'] > 0)
            reminders = Reminder.query.paginate(
                page, app.config['PER_PAGE'], False).items
    ...
    def get(self, car_id):
        ...
        '''for carData in car.carData:
            ...
            if (carData.carDataCode == 3) and (CarData.tagliando(car, carData.dataInt, CarData.GetKm(car, car.carData), CarData.GetDateDetection(car, car.carData))):
                ...
                response['check_km'] = reminder_schema.dump(
                    carData.reminder).get('text')
            ...
            if (carData.carDataCode == 4) and (CarData.assicurazione(car, carData.dataDate)):
                ...
                response['assurance_date'] = reminder_schema.dump(
                    carData.reminder).get('text')
            ...
            if (carData.carDataCode == 5) and (CarData.bollo(car, carData.dataDate)):
                ...
                response['tax_date'] = reminder_schema.dump(
                    carData.reminder).get('text')
...
@reminders.route('/user/<int:user_id>')
class User_reminders_requests(Resource):
    def get(self, user_id):
        '''get number of reminder for a user and their car id'''
        ...
        for car in user.cars:
            ...
            for car_data in car.carData:
                if car_data.reminder:
                    ...
                    cars[i]['reminders'] += 1
            i += 1
        response = {'reminders_count': count, 'cars': cars}
        return jsonify(response)
